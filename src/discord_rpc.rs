...
impl DiscordRpc {
    ...
    pub fn connect(&mut self, client_id: &str) -> Result<(), anyhow::Error> {
        log::debug!("DiscordRpc::connect called (enabled = {}, client_id = {})", self.enabled, client_id);
        if !self.enabled {
...
mod tests {
    ...
    fn test_rpc_connection() {
        ...
        rpc.connect("1089228496459345970").expect("Failed to connect to Discord RPC socket!");
